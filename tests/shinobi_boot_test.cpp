...
// first frame's RGBA pixels to /tmp/shinobi_frame.rgba so we can verify
...
//
// Writes `/tmp/shinobi_frame.rgba` (320*224*4 = 286 720 bytes) and prints
// a one-line summary.
//
...
public:
    explicit bus_adapter(shinobi::board* b) : m_board(b) {}
...
void run_main(shinobi::board& board, bus_adapter&, m68000_cpu& cpu,
              int cycles) {
    (void)board;
...
int main(int argc, char** argv) {
    ...
    const auto rl = shinobi::shinobi_rom_loader::load(rom_path);
    if (!rl) {
    ...
    std::printf("shinobi: loaded %s set=%d program=%zu bytes\n",
                shinobi::shinobi_rom_loader::set_display_name(rl.set),
                static_cast<int>(rl.set),
                rl.roms.program.size());

    auto board_ptr = std::make_unique<shinobi::board>();
    shinobi::board* board = board_ptr.get();
    ...
    constexpr int kCyclesPerFrame =
        shinobi::kMainCyclesPerFrame;   // 166 667 @ 10 MHz / 60 Hz
    ...
    std::ofstream out("/tmp/shinobi_frame.rgba", std::ios::binary);
    if (!out) {
        std::fprintf(stderr, "shinobi: cannot write /tmp/shinobi_frame.rgba\n");
        return 3;
    }
    ...
    {
        std::ofstream spr("/tmp/shinobi_sprram.bin", std::ios::binary);
    ...
    std::printf("shinobi: frames=%d PC=%08x framebuffer=%dx%d nonzero=%d/%d\n",
                n_frames,
                cpu.program_counter(),
                shinobi::kScreenW, shinobi::kScreenH, nz, total);
    std::printf("wrote /tmp/shinobi_frame.rgba\n");
    return 0;
}
