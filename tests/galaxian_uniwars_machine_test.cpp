// Native UniWar S integration test. The no-ROM section proves the Pisces
...
int main(int argc, char** argv) {
    {
        auto board = make_uniwars_board_interface();
        ...
        input_state input;
        input.coin1 = true;
        input.coin2 = true;  // UniWar S has no second coin bit on IN0.
        input.start = true;
        input.p2_start = true;
        ...
        assert((in1 & 0x08) != 0);
        assert((in1 & 0x10) != 0);
        assert(observed->cpu_read(0x7000, 0) == 0x01);
        assert(in0 == ports[0]);
        assert(in1 == ports[1]);

        observed->cpu_write(0x4000, 0x12);
        observed->cpu_write(0x5000, 0x34);
        observed->cpu_write(0x5400, 0x56);  // video RAM mirror
        observed->cpu_write(0x5800, 0x78);
        assert(observed->cpu_read(0x4000, 0) == 0x12);
        assert(observed->cpu_read(0x5000, 0) == 0x56);
        assert(observed->cpu_read(0x5400, 0) == 0x56);
        assert(observed->cpu_read(0x5800, 0) == 0x78);
    ...
    if (argc < 2) {
        std::puts("UniWar S board contract passed (no ROM integration path)");
        return 0;
    }
    ...
    auto sound = make_uniwars_sound_synth();
    sound->reset();
    ...
    std::printf("UniWar S: frames=%zu colors=%zu visible=%zu\n",
                unique_frames.size(), colors.size(), visible_pixels);
    std::printf("UniWar S sound: lines=%zu pitch=%zu lfo=%zu peak=%d audible=%zu\n",
