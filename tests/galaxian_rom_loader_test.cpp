...
//
// With no arguments: assert all known set names and return 0. With four
// arguments (phoenix_path galaxian_path mooncrst_path uniwars_path):
...
int main(int argc, char** argv) {
    ...
    assert(galaxian_rom_loader::set_display_name(
               galaxian_rom_set::galaxian) != nullptr);
    assert(galaxian_rom_loader::set_display_name(
    ...
    const std::string galaxian_path = argv[2];
    const std::string mooncrst_path = argv[3];
    const std::string uniwars_path = argv[4];
    ...
    assert(galaxian_rom_loader::identify_set(galaxian_path) ==
    ...
    // Original Namco Galaxian load: five program ROMs and two graphics ROMs.
    {
        const galaxian_rom_load_result result =
            galaxian_rom_loader::load(galaxian_path);
        if (!result) {
            std::fputs(result.error.c_str(), stderr);
            return 1;
        }
        assert(result.set == galaxian_rom_set::galaxian);
        assert(result.roms.complete());
        assert(any_nonzero(result.roms.program));
