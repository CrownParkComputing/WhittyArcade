...
#define f2u model1_f2u
#define u2f model1_u2f
...
void model1_tgp_hle::set_copro_data(const std::vector<uint8_t>& bytes)
{
    const std::size_t count = bytes.size() / 4;
    m_copro_data_storage.words.assign(count, 0);
    for (std::size_t index = 0; index < count; ++index) {
        const std::size_t offset = index * 4;
        m_copro_data_storage.words[index] =
...
void model1_tgp_hle::reset()
{
    m_input_storage.clear();
    m_output_storage.clear();
    std::fill(m_copro_ram_data.begin(), m_copro_ram_data.end(), 0);
    std::memset(m_cmat, 0, sizeof(m_cmat));
    m_cmat[0] = m_cmat[4] = m_cmat[8] = 1.0f;
    std::memset(m_mat_stack, 0, sizeof(m_mat_stack));
    std::memset(m_mat_vector, 0, sizeof(m_mat_vector));
    m_mat_stack_pos = 0;
    m_acc = 0;
    m_copro_hle_active_list_length = 0;
    m_copro_hle_active_list_pos = 0;
    m_copro_hle_list_length = 0;
    m_copro_hle_ram_scan_adr = 0;
    m_v60_copro_ram_adr = 0;
    m_v60_copro_ram_latch[0] = m_v60_copro_ram_latch[1] = 0;
    m_unimplemented_count = 0;
    m_last_unimplemented_function = 0;
    m_last_unimplemented_pc = 0;
}

void model1_tgp_hle::push(u32 value, u32 source_pc)
{
    m_pushpc = source_pc;
    m_copro_fifo_in->push(value);
    copro_hle_vf();
}

bool model1_tgp_hle::output_ready() const
{
    return !m_output_storage.is_empty();
}

u32 model1_tgp_hle::pop_output()
{
    return m_copro_fifo_out->pop();
}

u16 model1_tgp_hle::read_ram_half(unsigned half)
{
    const u32 value = m_copro_ram_data[m_v60_copro_ram_adr & 0x7fff];
    const u16 result = static_cast<u16>(value >> ((half & 1) * 16));
    if ((half & 1) && (m_v60_copro_ram_adr & 0x8000))
        ++m_v60_copro_ram_adr;
    return result;
}

void model1_tgp_hle::write_ram_half(unsigned half, u16 value)
{
    m_v60_copro_ram_latch[half & 1] = value;
    if (half & 1) {
        m_copro_ram_data[m_v60_copro_ram_adr & 0x7fff] =
            static_cast<u32>(m_v60_copro_ram_latch[0]) |
            (static_cast<u32>(m_v60_copro_ram_latch[1]) << 16);
        if (m_v60_copro_ram_adr & 0x8000) ++m_v60_copro_ram_adr;
    }
}

u32 model1_tgp_hle::next_random()
{
    m_random_state = m_random_state * 1664525U + 1013904223U;
    return m_random_state;
}

void model1_tgp_hle::fifoout_push(u32 data)
{
	m_copro_fifo_out->push(u32(data));
}

void model1_tgp_hle::fifoout_push_f(float data)
{
	m_copro_fifo_out->push(f2u(data));
}

u32 model1_tgp_hle::fifoin_pop()
{
	return m_copro_fifo_in->pop();
}

float model1_tgp_hle::fifoin_pop_f()
{
	return u2f(m_copro_fifo_in->pop());
}

float model1_tgp_hle::tcos(s16 a)
{
	if(a == 16384 || a == -16384)
		return 0;
	else if(a == -32768)
...
float model1_tgp_hle::ram_get_f()
{
	return u2f(m_copro_ram_data[(m_copro_hle_ram_scan_adr++) & 0x7fff]);
}

TGP_FUNCTION( model1_tgp_hle::fadd )
{
...
TGP_FUNCTION( model1_tgp_hle::fsub )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float r = a-b;
...
TGP_FUNCTION( model1_tgp_hle::fmul )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float r = a*b;
...
TGP_FUNCTION( model1_tgp_hle::fdiv )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
//  float r = !b ? 1e39 : a/b;
...
TGP_FUNCTION( model1_tgp_hle::matrix_push )
{
...
TGP_FUNCTION( model1_tgp_hle::matrix_pop )
{
	if(m_mat_stack_pos) {
		m_mat_stack_pos--;
...
TGP_FUNCTION( model1_tgp_hle::matrix_write )
{
	int i;
	for(i=0; i<12; i++)
...
TGP_FUNCTION( model1_tgp_hle::clear_stack )
{
	logerror("TGP clear_stack (%x)\n", m_pushpc);
	m_mat_stack_pos = 0;
}

TGP_FUNCTION( model1_tgp_hle::matrix_mul )
{
	float m[12];
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::anglev )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::normalize )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::acc_seti )
{
...
TGP_FUNCTION( model1_tgp_hle::track_select )
{
	int32_t a = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::load_base )
{
...
TGP_FUNCTION( model1_tgp_hle::transpose )
{
...
TGP_FUNCTION( model1_tgp_hle::anglep )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::matrix_read )
{
	int i;
...
TGP_FUNCTION( model1_tgp_hle::matrix_trans )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::matrix_scale )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::matrix_rotx )
{
	s16 a = fifoin_pop();
	float s = tsin(a);
	float c = tcos(a);
...
TGP_FUNCTION( model1_tgp_hle::matrix_roty )
{
	s16 a = fifoin_pop();
	float s = tsin(a);
	float c = tcos(a);
...
TGP_FUNCTION( model1_tgp_hle::matrix_rotz )
{
	s16 a = fifoin_pop();
	float s = tsin(a);
	float c = tcos(a);
...
TGP_FUNCTION( model1_tgp_hle::track_read_quad )
{
	u32 a = fifoin_pop();
	int offd;
...
TGP_FUNCTION( model1_tgp_hle::intercept )
{
...
TGP_FUNCTION( model1_tgp_hle::transform_point )
{
...
TGP_FUNCTION( model1_tgp_hle::fcos_m1 )
{
	s16 a = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::fsin_m1 )
{
	s16 a = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::fcosm_m1 )
{
	s16 a = fifoin_pop();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::fsinm_m1 )
{
	s16 a = fifoin_pop();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::distance3 )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::ftoi )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::itof )
{
	int32_t a = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::acc_set )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::acc_get )
{
	logerror("TGP acc_get (%x)\n", m_pushpc);
	fifoout_push_f(m_acc);
}

TGP_FUNCTION( model1_tgp_hle::acc_add )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::acc_sub )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::acc_mul )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::acc_div )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f42 )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::xyz2rqf )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f43_swa )
{
	float a = fifoin_pop_f();
	int b = fifoin_pop();
	int c = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::track_read_tri )
{
	u32 a = fifoin_pop();
	int offd;
...
TGP_FUNCTION( model1_tgp_hle::matrix_sdir )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::fsqrt )
{
	float a = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::vlength )
{
	float x = fifoin_pop_f();
	float y = fifoin_pop_f();
	float z = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f47 )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::track_read_info )
{
	u16 a = fifoin_pop();
	int offd;
...
TGP_FUNCTION( model1_tgp_hle::colbox_set )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::colbox_test )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f49_swa )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f50_swa )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f52 )
{
	logerror("TGP f52 (%x)\n", m_pushpc);
}

TGP_FUNCTION( model1_tgp_hle::matrix_rdir )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::track_lookup )
{
	float a = fifoin_pop_f();
	u32 b = fifoin_pop();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f56 )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::int_normal )
{
...
TGP_FUNCTION( model1_tgp_hle::matrix_readt )
{
...
TGP_FUNCTION( model1_tgp_hle::acc_geti )
{
	logerror("TGP acc_geti (%x)\n", m_pushpc);
	fifoout_push((int)m_acc);
}

TGP_FUNCTION( model1_tgp_hle::int_point )
{
...
TGP_FUNCTION( model1_tgp_hle::col_setcirc )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::col_testpt )
{
	float x, y;
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::push_and_ident )
{
	if(m_mat_stack_pos != MAT_STACK_SIZE) {
		memcpy(m_mat_stack[m_mat_stack_pos], m_cmat, sizeof(m_cmat));
		m_mat_stack_pos++;
...
TGP_FUNCTION( model1_tgp_hle::catmull_rom )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::distance )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::car_move )
{
	s16 a = fifoin_pop();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::cpa )
{
...
TGP_FUNCTION( model1_tgp_hle::vmat_store )
{
	u32 a = fifoin_pop();
	if(a<21)
...
TGP_FUNCTION( model1_tgp_hle::vmat_restore )
{
	u32 a = fifoin_pop();
	if(a<21)
...
TGP_FUNCTION( model1_tgp_hle::vmat_mul )
{
	u32 a = fifoin_pop();
	u32 b = fifoin_pop();
	if(a<21 && b<21) {
...
TGP_FUNCTION( model1_tgp_hle::vmat_read )
{
	u32 a = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::matrix_rtrans )
{
...
TGP_FUNCTION( model1_tgp_hle::matrix_unrot )
{
...
TGP_FUNCTION( model1_tgp_hle::f80 )
{
	logerror("TGP f80 (%x)\n", m_pushpc);
	//  m_cmat[9] = m_cmat[10] = m_cmat[11] = 0;
}

TGP_FUNCTION( model1_tgp_hle::vmat_save )
{
	u32 a = fifoin_pop();
	int i;
	logerror("TGP vmat_save 0x%x (%x)\n", a, m_pushpc);
	for(i=0; i<16; i++)
		memcpy(m_copro_ram_data.data()+a+0x10*i, m_mat_vector[i], sizeof(m_cmat));
}

TGP_FUNCTION( model1_tgp_hle::vmat_load )
{
	u32 a = fifoin_pop();
	int i;
	logerror("TGP vmat_load 0x%x (%x)\n", a, m_pushpc);
	for(i=0; i<16; i++)
		memcpy(m_mat_vector[i], m_copro_ram_data.data()+a+0x10*i, sizeof(m_cmat));
}

TGP_FUNCTION( model1_tgp_hle::ram_setadr )
{
	m_copro_hle_ram_scan_adr = fifoin_pop() - 0x8000;
	logerror("TGP f0 ram_setadr 0x%x (%x)\n", m_copro_hle_ram_scan_adr+0x8000, m_pushpc);
}

TGP_FUNCTION( model1_tgp_hle::groundbox_test )
{
...
TGP_FUNCTION( model1_tgp_hle::f89 )
{
	u32 a = fifoin_pop();
	u32 b = fifoin_pop();
	u32 c = fifoin_pop();
...
TGP_FUNCTION( model1_tgp_hle::f92 )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f93 )
{
	float a = fifoin_pop_f();
	(void)a;
...
TGP_FUNCTION( model1_tgp_hle::f94 )
{
	u32 a = fifoin_pop();
	(void)a;
...
TGP_FUNCTION( model1_tgp_hle::vmat_flatten )
{
	int i;
	float m[12];
...
TGP_FUNCTION( model1_tgp_hle::vmat_load1 )
{
	u32 a = fifoin_pop();
	logerror("TGP vmat_load1 0x%x (%x)\n", a, m_pushpc);
	memcpy(m_cmat, m_copro_ram_data.data()+a, sizeof(m_cmat));
}

TGP_FUNCTION( model1_tgp_hle::ram_trans )
{
	float a = ram_get_f();
	float b = ram_get_f();
	float c = ram_get_f();
...
TGP_FUNCTION( model1_tgp_hle::f98 )
{
	u32 a = fifoin_pop();
	(void)a;
...
TGP_FUNCTION( model1_tgp_hle::f99 )
{
	logerror("TGP f99 (%x)\n", m_pushpc);
}

TGP_FUNCTION( model1_tgp_hle::f100 )
{
	int i;
	logerror("TGP f100 get list (%x)\n", m_pushpc);
	for(i=0; i<m_copro_hle_list_length; i++)
		fifoout_push_f((next_random() % 1000)/100.0);
}

TGP_FUNCTION( model1_tgp_hle::groundbox_set )
{
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
	float c = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f102 )
{
	float px, py, pz;
	float a = fifoin_pop_f();
	float b = fifoin_pop_f();
...
TGP_FUNCTION( model1_tgp_hle::f103 )
{
	m_copro_hle_ram_scan_adr = fifoin_pop() - 0x8000;
...
const struct model1_tgp_hle::function model1_tgp_hle::ftab_vf[] = {
	{ &model1_tgp_hle::fadd,            2 }, /* 0x00 */ // 0b5 09b
	{ &model1_tgp_hle::fsub,            2 },            // 0ba 0a0
	{ &model1_tgp_hle::fmul,            2 },            // 0bf 0a5
	{ &model1_tgp_hle::fdiv,            2 },            // 0c4 0aa
	{ nullptr,                        0 },            // 0d2 0b9
	{ &model1_tgp_hle::matrix_push,     0 },            // 0e3 0cc
	{ &model1_tgp_hle::matrix_pop,      0 },            // 0f7 0e0
	{ &model1_tgp_hle::matrix_write,   12 },            // 106 0ef
	{ &model1_tgp_hle::clear_stack,     0 },            // 10a 0f4
	{ &model1_tgp_hle::matrix_mul,     12 },            // 10e 0f8
	{ &model1_tgp_hle::anglev,          2 },            // 154 13f
	{ nullptr,                        0 },            // 15d 149
	{ nullptr,                        0 },            // 19f 18a
	{ &model1_tgp_hle::track_select,    1 },            // 1b8 1a5
	{ &model1_tgp_hle::load_base,       4 },            // 1bb 1a8
	{ &model1_tgp_hle::anglep,          4 },            // 1c1 1ae

	{ &model1_tgp_hle::matrix_ident,    0 }, /* 0x10 */ // 1d2 1c1
	{ &model1_tgp_hle::matrix_read,     0 },            // 1d9 1c8
	{ &model1_tgp_hle::matrix_trans,    3 },            // 1dd 1d1
	{ &model1_tgp_hle::matrix_scale,    3 },            // 1f3 1e8
	{ &model1_tgp_hle::matrix_rotx,     1 },            // 20a 200
	{ &model1_tgp_hle::matrix_roty,     1 },            // 223 21a
	{ &model1_tgp_hle::matrix_rotz,     1 },            // 23b 233
	{ nullptr,                        0 },            // 253 24c
	{ &model1_tgp_hle::track_read_quad, 1 },            // 2aa 2a8
	{ nullptr,                        0 },            // 2b0 2b0
	{ &model1_tgp_hle::transform_point, 3 },            // 2b2 2b4
	{ &model1_tgp_hle::fsin_m1,         1 },            // 2cb 2d7
	{ &model1_tgp_hle::fcos_m1,         1 },            // 2ce 2df
	{ &model1_tgp_hle::fsinm_m1,        2 },            // 2d1 2e7
	{ &model1_tgp_hle::fcosm_m1,        2 },            // 2d7 2f1
	{ &model1_tgp_hle::distance3,       6 },            // 2dd 2fb

	{ nullptr,                        0 }, /* 0x20 */ // 2ff 31e
	{ nullptr,                        0 },            // 2ff 31e
	{ nullptr,                        0 },            // 300 31f
	{ nullptr,                        0 },            // 304 323
	{ &model1_tgp_hle::acc_set,         1 },            // 308 327
	{ &model1_tgp_hle::acc_get,         0 },            // 30a 32a
	{ &model1_tgp_hle::acc_add,         1 },            // 30c 32e
	{ &model1_tgp_hle::acc_sub,         1 },            // 311 333
	{ &model1_tgp_hle::acc_mul,         1 },            // 316 338
	{ &model1_tgp_hle::acc_div,         1 },            // 31b 33d
	{ &model1_tgp_hle::f42,             3 },            // 329 34b
	{ &model1_tgp_hle::f43,             6 },            // 36c 38f
	{ &model1_tgp_hle::track_read_tri,  1 },            // 3c2 3e2
	{ &model1_tgp_hle::fsqrt,           1 },            // 3c7 3e9
	{ &model1_tgp_hle::vlength,         3 },            // 3cf 3f2
	{ nullptr,                        0 },            // 3ef 411

	{ &model1_tgp_hle::track_read_info, 1 }, /* 0x30 */ // 410 431
	{ &model1_tgp_hle::colbox_set,     12 },            // 413 436
	{ &model1_tgp_hle::colbox_test,     3 },            // 417 43b
	{ nullptr,                        0 },            // 43b 463
	{ nullptr,                        0 },            // 44d 478
	{ nullptr,                        0 },            // 452 47d
	{ &model1_tgp_hle::track_lookup,    4 },            // 457 482
	{ nullptr,                        0 },            // 51a 522
	{ nullptr,                        0 },            // 521 527
	{ nullptr,                        0 },            // 52f 535
	{ nullptr,                        0 },            // 53d 543
	...
	{ &model1_tgp_hle::col_setcirc,     3 }, /* 0x40 */ // 5f3 5fe
	{ &model1_tgp_hle::col_testpt,      2 },            // 5fa 603
	{ nullptr,                        0 },            // 615 61e
	{ &model1_tgp_hle::distance,        4 },            // 631 639
	{ nullptr,                        0 },            // 63f 648
	{ nullptr,                        0 },            // 643 64e
	{ nullptr,                        0 },            // 64b 657
	{ &model1_tgp_hle::car_move,        4 },            // 661 66e
	{ &model1_tgp_hle::cpa,            12 },            // 7d9
	{ nullptr,                        0 },
	{ &model1_tgp_hle::vmat_store,      1 },
	{ &model1_tgp_hle::vmat_restore,    1 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::vmat_mul,        2 },
	{ &model1_tgp_hle::vmat_read,       1 },
	{ &model1_tgp_hle::matrix_unrot,    0 },

	{ &model1_tgp_hle::f80,             0 }, /* 0x50 */
	{ nullptr,                        0 },
	{ &model1_tgp_hle::matrix_rtrans,   0 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::vmat_save,       1 },
	{ &model1_tgp_hle::vmat_load,       1 },
	{ &model1_tgp_hle::ram_setadr,      1 },
	{ &model1_tgp_hle::groundbox_test,  3 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::f89,             4 },
	{ nullptr,                        0 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::f92,             4 },
	{ &model1_tgp_hle::f93,             1 },
	{ &model1_tgp_hle::f94,             1 },
	{ &model1_tgp_hle::vmat_flatten,    0 },

	{ &model1_tgp_hle::vmat_load1,      1 }, /* 0x60 */
	{ &model1_tgp_hle::ram_trans,       0 },
	{ &model1_tgp_hle::f98,             1 },
	{ &model1_tgp_hle::f99,             0 },
	{ &model1_tgp_hle::f100,            0 },
	{ &model1_tgp_hle::groundbox_set,   7 },
	{ &model1_tgp_hle::f102,            8 },
	{ &model1_tgp_hle::f103,            1 }
};

// Used in swa scene 1 and unemulated:
//   f49_swa

const struct model1_tgp_hle::function model1_tgp_hle::ftab_swa[] = {
	{ &model1_tgp_hle::fadd,            2 },  /* 0x00 */
	{ &model1_tgp_hle::fsub,            2 },
	{ &model1_tgp_hle::fmul,            2 },
	{ &model1_tgp_hle::fdiv,            2 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::matrix_push,     0 },
	{ &model1_tgp_hle::matrix_pop,      0 },
	{ &model1_tgp_hle::matrix_write,   12 },
	{ &model1_tgp_hle::clear_stack,     0 },
	{ &model1_tgp_hle::matrix_mul,     12 },
	{ &model1_tgp_hle::anglev,          2 },
	{ &model1_tgp_hle::triangle_normal, 9 },
	{ &model1_tgp_hle::normalize,       3 },
	{ &model1_tgp_hle::acc_seti,        1 },
	{ &model1_tgp_hle::load_base,       4 },
	{ &model1_tgp_hle::transpose,       0 },

	{ &model1_tgp_hle::matrix_ident,    0 }, /* 0x10 */
	{ &model1_tgp_hle::matrix_read,     0 },
	{ &model1_tgp_hle::matrix_trans,    3 },
	{ &model1_tgp_hle::matrix_scale,    3 },
	{ &model1_tgp_hle::matrix_rotx,     1 },
	{ &model1_tgp_hle::matrix_roty,     1 },
	{ &model1_tgp_hle::matrix_rotz,     1 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::intercept,       7 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::transform_point, 3 },
	{ &model1_tgp_hle::fsin_m1,         1 },
	{ &model1_tgp_hle::fcos_m1,         1 },
	{ &model1_tgp_hle::fsinm_m1,        2 },
	{ &model1_tgp_hle::fcosm_m1,        2 },
	{ &model1_tgp_hle::distance3,       6 },

	{ nullptr,                        0 }, /* 0x20 */
	{ nullptr,                        0 },
	{ &model1_tgp_hle::ftoi,            1 },
	{ &model1_tgp_hle::itof,            1 },
	{ &model1_tgp_hle::acc_set,         1 },
	{ &model1_tgp_hle::acc_get,         0 },
	{ &model1_tgp_hle::acc_add,         1 },
	{ &model1_tgp_hle::acc_sub,         1 },
	{ &model1_tgp_hle::acc_mul,         1 },
	{ &model1_tgp_hle::acc_div,         1 }, // not used ?
	{ &model1_tgp_hle::xyz2rqf,         3 },
	{ &model1_tgp_hle::f43_swa,         3 },
	{ &model1_tgp_hle::matrix_sdir,     3 },
	{ &model1_tgp_hle::fsqrt,           1 },
	{ &model1_tgp_hle::vlength,         3 },
	{ &model1_tgp_hle::f47,             3 },

	{ nullptr,                        0 }, /* 0x30 */
	{ &model1_tgp_hle::f49_swa,         6 },
	{ &model1_tgp_hle::f50_swa,         4 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::f52,             0 },
	{ &model1_tgp_hle::matrix_rdir,     3 },
	{ nullptr,                        0 },
	{ nullptr,                        0 },
	{ &model1_tgp_hle::f56,             7 },
	{ &model1_tgp_hle::int_point,       0 },
	{ &model1_tgp_hle::matrix_readt,    0 },
	{ &model1_tgp_hle::acc_geti,        0 },
	{ &model1_tgp_hle::int_normal,      0 },
	{ nullptr,                        0 },
	{ nullptr,                        0 },
	{ nullptr,                        0 },
	{ nullptr,                        0 },

	{ &model1_tgp_hle::push_and_ident,  0 }, /* 0x40 */
	{ nullptr,                        0 },
	{ &model1_tgp_hle::catmull_rom,    13 }
};

void model1_tgp_hle::copro_hle_vf()
{
 ...
 next:
	...
	if(f < ARRAY_LENGTH(ftab_vf) && ftab_vf[f].cb != nullptr) {
		...
		m_last_unimplemented_function = f;
		m_last_unimplemented_pc = m_pushpc;
		logerror("TGP function %d unimplemented (%x)\n", f, m_pushpc);
		m_copro_fifo_in->pop();
	}
}

void model1_tgp_hle::copro_hle_swa()
{
...
#if 0 // MAME device/LLE plumbing below is intentionally not part of this core.
void model1_tgp_hle::machine_start()
{
	m_digits.resolve();
	m_outs.resolve();
...
void model1_tgp_hle::copro_reset()
{
	m_v60_copro_ram_adr = 0;
	m_copro_ram_adr = 0;
...
READ16_MEMBER(model1_tgp_hle::v60_copro_ram_adr_r)
{
	return m_v60_copro_ram_adr;
}

WRITE16_MEMBER(model1_tgp_hle::v60_copro_ram_adr_w)
{
	COMBINE_DATA(&m_v60_copro_ram_adr);
}

READ16_MEMBER(model1_tgp_hle::v60_copro_ram_r)
{
	u16 r;
...
WRITE16_MEMBER(model1_tgp_hle::v60_copro_ram_w)
{
...
READ16_MEMBER(model1_tgp_hle::v60_copro_fifo_r)
{
	if (!offset) {
...
WRITE16_MEMBER(model1_tgp_hle::v60_copro_fifo_w)
{
	if(offset) {
...
void model1_tgp_hle::copro_prog_map(address_map &map)
{
	map(0x000, 0x7ff).rom();
}

void model1_tgp_hle::copro_data_map(address_map &map)
{
...
void model1_tgp_hle::copro_io_map(address_map &map)
{
	map(0x0008, 0x0008).rw(FUNC(model1_tgp_hle::copro_ramadr_r), FUNC(model1_tgp_hle::copro_ramadr_w));
	map(0x0009, 0x0009).rw(FUNC(model1_tgp_hle::copro_ramdata_r), FUNC(model1_tgp_hle::copro_ramdata_w));
	map(0x0020, 0x0023).rw(FUNC(model1_tgp_hle::copro_sincos_r), FUNC(model1_tgp_hle::copro_sincos_w));
	map(0x0024, 0x0027).rw(FUNC(model1_tgp_hle::copro_atan_r), FUNC(model1_tgp_hle::copro_atan_w));
	map(0x0028, 0x0029).rw(FUNC(model1_tgp_hle::copro_inv_r), FUNC(model1_tgp_hle::copro_inv_w));
	map(0x002a, 0x002b).rw(FUNC(model1_tgp_hle::copro_isqrt_r), FUNC(model1_tgp_hle::copro_isqrt_w));
	map(0x002e, 0x002e).w(FUNC(model1_tgp_hle::copro_data_w));
	map(0x8000, 0xffff).r(FUNC(model1_tgp_hle::copro_data_r));
}

void model1_tgp_hle::copro_rf_map(address_map &map)
{
	map(0x0, 0x0).nopw(); // leds? busy flag?
}

WRITE32_MEMBER(model1_tgp_hle::copro_ramadr_w)
{
	COMBINE_DATA(&m_copro_ram_adr);
}

READ32_MEMBER(model1_tgp_hle::copro_ramadr_r)
{
	return m_copro_ram_adr;
}

WRITE32_MEMBER(model1_tgp_hle::copro_ramdata_w)
{
	COMBINE_DATA(&m_copro_ram_data[m_copro_ram_adr & 0x7fff]);
	m_copro_ram_adr ++;
}

READ32_MEMBER(model1_tgp_hle::copro_ramdata_r)
{
...
WRITE32_MEMBER(model1_tgp_hle::copro_sincos_w)
{
	COMBINE_DATA(&m_copro_sincos_base);
}

READ32_MEMBER(model1_tgp_hle::copro_sincos_r)
{
...
WRITE32_MEMBER(model1_tgp_hle::copro_inv_w)
{
	COMBINE_DATA(&m_copro_inv_base);
}

READ32_MEMBER(model1_tgp_hle::copro_inv_r)
{
...
WRITE32_MEMBER(model1_tgp_hle::copro_isqrt_w)
{
	COMBINE_DATA(&m_copro_isqrt_base);
}

READ32_MEMBER(model1_tgp_hle::copro_isqrt_r)
{
...
WRITE32_MEMBER(model1_tgp_hle::copro_atan_w)
{
	COMBINE_DATA(&m_copro_atan_base[offset]);
}

READ32_MEMBER(model1_tgp_hle::copro_atan_r)
{
...
WRITE32_MEMBER(model1_tgp_hle::copro_data_w)
{
	COMBINE_DATA(&m_copro_data_base);
}

READ32_MEMBER(model1_tgp_hle::copro_data_r)
{
