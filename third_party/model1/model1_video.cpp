...
#define LOG_TGP(x) ((void)0)
#define LOG_TGP_DEV(d,x) ((void)0)
#define u2f model1_video_u2f
...
model1_video::model1_video()
{
    video_start();
}

void model1_video::set_polygon_rom(const std::vector<uint8_t>& bytes)
{
    m_poly_rom.assign(bytes.size() / 4, 0);
    for (std::size_t index = 0; index < m_poly_rom.size(); ++index) {
        const std::size_t offset = index * 4;
        m_poly_rom[index] = static_cast<uint32_t>(bytes[offset]) |
...
uint32_t model1_video::readi(int adr) const
{
	return m_display_list_current[(adr + 0)&0x7fff] | (m_display_list_current[(adr + 1)&0x7fff] << 16);
}

int16_t model1_video::readi16(int adr) const
{
	return m_display_list_current[(adr + 0)&0x7fff];
}

float model1_video::readf(int adr) const
{
	return u2f(readi(adr));
}

void model1_video::view_t::transform_point(point_t *p) const
{
	point_t q = *p;
...
void model1_video::view_t::transform_vector(glm::vec3& p) const
{
	glm::vec3 q(p);
...
void model1_video::cross_product(point_t* o, const point_t* p, const point_t* q) const
{
	o->x = p->y * q->z - q->y * p->z;
	o->y = p->z * q->x - q->z * p->x;
	o->z = p->x * q->y - q->x * p->y;
...
float model1_video::view_determinant(const point_t *p1, const point_t *p2, const point_t *p3) const
{
	float x1 = p2->x - p1->x;
	float y1 = p2->y - p1->y;
	float z1 = p2->z - p1->z;
...
void model1_video::view_t::project_point(point_t *p) const
{
	p->xx = p->x / p->z;
	p->yy = p->y / p->z;
...
void model1_video::view_t::project_point_direct(point_t *p) const
{
	p->xx = p->x /*/ p->z*/;
	p->yy = p->y /*/ p->z*/;
	p->s.x = xc + p->xx;
...
void model1_video::draw_hline(bitmap_rgb32 &bitmap, int x1, int x2, int y, int color)
{
...
void model1_video::draw_hline_moired(bitmap_rgb32 &bitmap, int x1, int x2, int y, int color)
{
	uint32_t *base = &bitmap.pix32(y);
	while(x1 <= x2)
	{
...
void model1_video::fill_slope(bitmap_rgb32 &bitmap, view_t *view, int color, int32_t x1, int32_t x2, int32_t sl1, int32_t sl2, int32_t y1, int32_t y2, int32_t *nx1, int32_t *nx2)
{
	if(y1 > view->y2)
	{
		return;
...
void model1_video::fill_line(bitmap_rgb32 &bitmap, view_t *view, int color, int32_t y, int32_t x1, int32_t x2)
{
	int xx1 = x1>>FRAC_SHIFT;
	int xx2 = x2>>FRAC_SHIFT;
...
void model1_video::fill_quad(bitmap_rgb32 &bitmap, view_t *view, const quad_t& q) const
{
	spoint_t p[8];
	int color = q.col;
...
#if 0
void model1_video::draw_line(bitmap_rgb32 &bitmap, model1_video::view_t *view, int color, int x1, int y1, int x2, int y2) const
{
...
{
	model1_video::quad_t* const* c1 = static_cast<model1_video::quad_t* const*>(q1);
	model1_video::quad_t* const* c2 = static_cast<model1_video::quad_t* const*>(q2);

	return (*c1)->compare(*c2);
}

int model1_video::quad_t::compare(const model1_video::quad_t* other) const
{
	float z2 = other->z;
...
void model1_video::sort_quads() const
{
	...
	qsort(m_quadind, count, sizeof(model1_video::quad_t*), comp_quads);
}

void model1_video::unsort_quads() const
{
	const int count = m_quadpt - m_quaddb;
	for (int i = 0; i < count; i++)
	{
...
void model1_video::draw_quads(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
#if 0
uint16_t model1_video::scale_color(uint16_t color, float level) const
{
...
void model1_video::view_t::recompute_frustum()
{
...
bool model1_video::fclip_isc_near(view_t *, point_t *p)
{
	// A finite positive near plane is required before the side planes.  The
	// original renderer projected behind-camera vertices to (0,0), which made
	// road or vehicle polygons crossing the camera either disappear or form a
	// detached spike elsewhere on screen.
	return !std::isfinite(p->z) || p->z <= 0.001f;
}

void model1_video::fclip_clip_near(view_t *view, point_t *pt,
	point_t *p1, point_t *p2)
{
	constexpr float near_z = 0.001f;
	const float denominator = p1->z - p2->z;
	const float t = denominator == 0.0f ? 0.0f :
		(near_z - p2->z) / denominator;
	pt->x = p1->x * t + p2->x * (1.0f - t);
	pt->y = p1->y * t + p2->y * (1.0f - t);
	pt->z = near_z;
	view->project_point(pt);
}

bool model1_video::fclip_isc_bottom(view_t *view, point_t *p)
{
	return p->y > (p->z * view->a_bottom);
}

void model1_video::fclip_clip_bottom(view_t *view, point_t *pt, point_t *p1, point_t *p2)
{
...
bool model1_video::fclip_isc_top(view_t *view, point_t *p)
{
	return p->y < (p->z * view->a_top);
}

void model1_video::fclip_clip_top(view_t *view, point_t *pt, point_t *p1, point_t *p2)
{
...
bool model1_video::fclip_isc_left(view_t *view, point_t *p)
{
	return p->x < (p->z * view->a_left);
}

void model1_video::fclip_clip_left(view_t *view, point_t *pt, point_t *p1, point_t *p2)
{
...
bool model1_video::fclip_isc_right(view_t *view, point_t *p)
{
	return p->x > (p->z * view->a_right);
}

void model1_video::fclip_clip_right(view_t *view, point_t *pt, point_t *p1, point_t *p2)
{
...
void model1_video::fclip_push_quad_next(int level, quad_t& q, point_t *p1, point_t *p2, point_t *p3, point_t *p4)
{
	quad_t cquad(q.col, q.z, p1, p2, p3, p4);
	fclip_push_quad(level+1, cquad);
}

void model1_video::fclip_push_quad(int level, quad_t& q)
{
	view_t *view = m_view.get();
...
float model1_video::min4f(float a, float b, float c, float d)
{
	float m = a;
	if(b<m)
		m = b;
...
float model1_video::max4f(float a, float b, float c, float d)
{
	float m = a;
	if (b > m)
		m = b;
...
#endif
float model1_video::compute_specular(glm::vec3& normal, glm::vec3& light, float diffuse, int lmode)
{
#if 0
...
void model1_video::push_object(uint32_t tex_adr, uint32_t poly_adr,
                               uint32_t size, float& old_z) {
	...
	if (poly_adr & 0x800000)
		poly_data = reinterpret_cast<float *>(m_poly_ram.data());
	else
		poly_data = reinterpret_cast<float *>(m_poly_rom.data());
	...
	for (int i = 0; i < size; i++)
	{
		...
		// Type 2 records contain a single new vertex. Reading the following
		// record as a second vertex creates enormous stray polygons once the
		// road/cockpit wire geometry becomes active.
		if (type == 2)
			*p1 = *p0;
...
int model1_video::push_direct(int list_offset) {
	...
	//m_view-transform_point(old_p0);
	//m_view->transform_point(old_p1);
	// Direct polygons already contain screen-space coordinates; z is sorting
	// metadata and must not decide whether x/y are projected.
	...
	for (;;) {
		...
		// These are screen-space polys. The perspective frustum's z-scaled
		// tests can collapse them onto the origin; raster clipping still
		// scissors them to the active viewport.
		*m_quadpt++ = cquad;
...
int model1_video::skip_direct(int list_offset) const
{
	list_offset += 18;
...
void model1_video::draw_objects(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	m_last_quad_count += static_cast<std::size_t>(m_quadpt - m_quaddb);
...
int model1_video::draw_direct(bitmap_rgb32 &bitmap, const rectangle &cliprect, int list_offset)
{
...
void model1_video::set_current_render_list()
{
	if(!(m_listctl[0] & 4))
		m_listctl[0] = (m_listctl[0] & ~0x40) | (m_listctl[0] & 8 ? 0x40 : 0);
	m_display_list_current = m_listctl[0] & 0x40 ?
		m_display_list1.data() : m_display_list0.data();
}

int model1_video::get_list_number()
{
	if(!(m_listctl[0] & 4))
		m_listctl[0] = (m_listctl[0] & ~0x40) | (m_listctl[0] & 8 ? 0x40 : 0);
...
void model1_video::end_frame()
{
	if((m_listctl[0] & 4) && (m_frame_number & 1))
		m_listctl[0] ^= 0x40;
}

#if 0
READ16_MEMBER(model1_video::model1_listctl_r)
{
	if(!offset)
...
WRITE16_MEMBER(model1_video::model1_listctl_w)
{
...
void model1_video::view_t::init_translation_matrix()
{
...
void model1_video::view_t::set_viewport(float xcenter, float ycenter, float xl, float xr, float yb, float yt)
{
	xc = xcenter;
	yc = ycenter;
	x1 = xl;
...
void model1_video::view_t::set_lightparam(int index, float diffuse, float ambient, float specular, int power)
{
...
void model1_video::view_t::set_zoom(float x, float y)
{
	zoomx = x;
	zoomy = y;
...
void model1_video::view_t::set_light_direction(float x, float y, float z)
{
	light = glm::normalize(glm::vec3(x, y, z));
}

void model1_video::view_t::set_translation_matrix(float* mat)
{
	for (int i = 0; i < 12; i++)
	{
		translation[i] = mat[i];
...
void model1_video::view_t::set_view_translation(float x, float y)
{
	viewx = x;
	viewy = y;
...
void model1_video::tgp_render(bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	m_last_quad_count = 0;
	m_last_unknown_list_type = 0;
	m_render_done = 1;
	if ((m_listctl[1] & 0x1f) == 0x1f)
	{
		...
		for (;;) {
			...
			case 0x41:
				...
				if (true || zz >= 666)
					push_object(readi(list_offset + 2), readi(list_offset + 4),
					            readi(list_offset + 6), old_z);
				list_offset += 8;
				break;
			case 2:
			{
			...
			case 0xf:
				...
				goto end;
			default:
				m_last_unknown_list_type = static_cast<uint32_t>(type);
...
void model1_video::video_start()
{
	m_view = std::make_unique<model1_video::view_t>();
	m_poly_ram.assign(0x400000, 0);
	m_tgp_ram.assign(0x100000 - 0x40000, 0);
	// The original device reserved its worst-case maximum. Dynamic vectors
	// retain stable storage for a frame while avoiding a permanent 100+ MiB
	// allocation in the multi-board frontend.
	m_point_storage.resize(400000);
	m_quad_storage.resize(200000);
	m_quad_index_storage.resize(200000);
	m_pointdb = m_point_storage.data();
	m_quaddb = m_quad_storage.data();
	m_quadind = m_quad_index_storage.data();

	m_pointpt = m_pointdb;
	m_quadpt = m_quaddb;
	m_listctl[0] = m_listctl[1] = 0;

	m_clipfn[0].m_isclipped = &model1_video::fclip_isc_near;
	m_clipfn[0].m_clip = &model1_video::fclip_clip_near;
	m_clipfn[1].m_isclipped = &model1_video::fclip_isc_bottom;
	m_clipfn[1].m_clip = &model1_video::fclip_clip_bottom;
	m_clipfn[2].m_isclipped = &model1_video::fclip_isc_top;
	m_clipfn[2].m_clip = &model1_video::fclip_clip_top;
	m_clipfn[3].m_isclipped = &model1_video::fclip_isc_left;
	m_clipfn[3].m_clip = &model1_video::fclip_clip_left;
	m_clipfn[4].m_isclipped = &model1_video::fclip_isc_right;
	m_clipfn[4].m_clip = &model1_video::fclip_clip_right;

}

void model1_video::render(const std::vector<uint8_t>& display_list_0,
...
{
	auto bytes_to_words = [](const std::vector<uint8_t>& source,
								std::vector<uint16_t>& destination) {
		destination.assign(source.size() / 2, 0);
		for (std::size_t index = 0; index < destination.size(); ++index)
			destination[index] = static_cast<uint16_t>(source[index * 2]) |
				(static_cast<uint16_t>(source[index * 2 + 1]) << 8);
	};
	bytes_to_words(display_list_0, m_display_list0);
	bytes_to_words(display_list_1, m_display_list1);
	bytes_to_words(palette, m_paletteram16);
	bytes_to_words(color_translation, m_color_xlat);
	m_listctl[0] = list_control_0;
	m_listctl[1] = list_control_1;

	bitmap_rgb32& bitmap = m_bitmap;
	const rectangle cliprect{0, width - 1, 0, height - 1};
	model1_video::view_t *view = m_view.get();
#if 0
	{
		bool mod = false;
		double delta = 1.0;
	...
	if (!m_paletteram16.empty()) {
		const uint16_t color = m_paletteram16[0];
		background = (static_cast<uint32_t>(pal5bit(color & 31)) << 16) |
			(static_cast<uint32_t>(pal5bit((color >> 5) & 31)) << 8) |
			pal5bit((color >> 10) & 31);
	}
	bitmap.fill(background, cliprect);

	// Model 1 reuses Sega's System 24 four-page RAM tile generator. Pair B is
	// the opaque backdrop; pair A category zero sits below the TGP scene.
	draw_tile_pair(bitmap, tile_ram, character_ram, 1, -1, true);
	draw_tile_pair(bitmap, tile_ram, character_ram, 0, 0, false);
	tgp_render(bitmap, cliprect);
	// Category one supplies the foreground and HUD for both page pairs.
	draw_tile_pair(bitmap, tile_ram, character_ram, 1, 1, false);
	draw_tile_pair(bitmap, tile_ram, character_ram, 0, 1, false);
	tgp_scan();
	end_frame();
	++m_frame_number;

	m_rgba_pixels.resize(static_cast<std::size_t>(width * height * 4));
	for (std::size_t index = 0; index < bitmap.pixels().size(); ++index) {
		const uint32_t color = bitmap.pixels()[index];
		m_rgba_pixels[index * 4 + 0] = static_cast<uint8_t>(color >> 16);
		m_rgba_pixels[index * 4 + 1] = static_cast<uint8_t>(color >> 8);
		m_rgba_pixels[index * 4 + 2] = static_cast<uint8_t>(color);
		m_rgba_pixels[index * 4 + 3] = 0xff;
	}
}

void model1_video::draw_tile_pair(
	bitmap_rgb32& bitmap, const std::vector<uint8_t>& tile_ram,
	const std::vector<uint8_t>& character_ram, int pair, int category,
	bool opaque) const
{
	system24_draw_tile_pair(
		&bitmap.pix32(0), width, height, tile_ram, character_ram,
		pair, category, opaque,
		[this](uint16_t entry, unsigned pen) {
			const unsigned palette_index =
				((entry >> 7) & 0xff) * 16 + pen;
			if (palette_index >= m_paletteram16.size()) return uint32_t{0};
			const uint16_t color = m_paletteram16[palette_index];
			return (static_cast<uint32_t>(pal5bit(color & 31)) << 16) |
				(static_cast<uint32_t>(pal5bit((color >> 5) & 31)) << 8) |
				pal5bit((color >> 10) & 31);
		});
	return;

	auto word = [&tile_ram](std::size_t offset) -> uint16_t {
		const std::size_t byte = offset * 2;
		if (byte + 1 >= tile_ram.size()) return 0;
		return static_cast<uint16_t>(tile_ram[byte]) |
			(static_cast<uint16_t>(tile_ram[byte + 1]) << 8);
	};
	if (pair < 0 || pair > 1 || category < -1 || category > 1 ||
		character_ram.size() < 32 || m_paletteram16.size() < 0x2000)
		return;

	const int base_page = pair * 2;
	const uint16_t pair_hscroll = word(0x5000 + base_page);
	const uint16_t pair_vscroll = word(0x5004 + base_page);
	...
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			if (mode == 0) {
				const uint16_t mask = word(mask_base + y * 4 + x / 128);
				const bool selected =
					(mask & (uint16_t{0x8000} >> ((x & 127) >> 3))) != 0;
				if (selected) ++page;
				hscroll_word = word(0x5000 + page);
				vscroll_word = word(0x5004 + page);
				if (vscroll_word & 0x8000) continue;
				if (hscroll_word & 0x8000)
					hscroll_word = word(0x4000 + 0x200 * page + y);
			} else if (mode == 1) {
				// A vertical screen split chooses one of the paired pages above
				// and the other below the split line.
				if (((-static_cast<int>(pair_vscroll)) & 0x200) == 0)
					page ^= 1;
				if (y >= ((-static_cast<int>(pair_vscroll)) & 0x1ff))
					page ^= 1;
				if (pair_hscroll & 0x8000)
					hscroll_word = word(0x4000 + 0x200 * base_page + y);
			} else {
				// Types two and three split horizontally. With line scroll, each
				// scanline supplies both its split position and page-select bit.
				if (pair_hscroll & 0x8000)
					hscroll_word = word(0x4000 + 0x200 * base_page + y);
			...
			const std::size_t tile_offset = static_cast<std::size_t>(page) * 0x1000 +
				static_cast<std::size_t>(scroll_y >> 3) * 64 + (source_x >> 3);
			const uint16_t entry = word(tile_offset);
			if (category >= 0 && ((entry >> 15) & 1) != category) continue;
			...
			// Character words are little-endian on the bus, while each packed
			// row presents the most-significant nibble first on screen.
			...
			const unsigned palette_index = ((entry >> 7) & 0xff) * 16 + pen;
			if (palette_index >= m_paletteram16.size()) continue;
			const uint16_t color = m_paletteram16[palette_index];
			bitmap.pix32(y, x) =
				(static_cast<uint32_t>(pal5bit(color & 31)) << 16) |
				(static_cast<uint32_t>(pal5bit((color >> 5) & 31)) << 8) |
				pal5bit((color >> 10) & 31);
		}
	}
