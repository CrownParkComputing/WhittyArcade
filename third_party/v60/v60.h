...
public:
	...
	// Framework-free host API. Memory callbacks are byte-wide because Model 1
	// has several mixed-width devices on the V60 bus; the core assembles
	// little-endian words and dwords exactly as the CPU observes them.
	...
	uint32_t program_counter() const { return m_reg[32]; }
	uint32_t register_value(unsigned index) const {
		return index < 68 ? m_reg[index] : 0;
	}
