...
v60_device::v60_device()
	: v60_device(machine_config{}, "standalone_v60", nullptr, 16000000)
{
}
...
void v60_device::standalone_start()
{
	device_start();
	device_reset();
}

void v60_device::standalone_reset()
{
	device_reset();
}

int v60_device::standalone_execute(int cycles)
{
	if (cycles <= 0)
		return 0;
	m_icount = cycles;
...
void v60_device::set_program_callbacks(read_callback read,
										 write_callback write)
{
	space(AS_PROGRAM).set_callbacks(std::move(read), std::move(write));
}

void v60_device::set_io_callbacks(read_callback read, write_callback write)
{
	space(AS_IO).set_callbacks(std::move(read), std::move(write));
}

std::string v60_device::disassemble(uint32_t address, std::size_t* length) const
{
	v60_disassembler disassembler;
	util::disasm_interface::data_buffer bytes(
		[this](offs_t location) { return space(AS_PROGRAM).read_byte(location); });
