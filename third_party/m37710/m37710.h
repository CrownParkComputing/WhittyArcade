...
class m37710_cpu_device
{
public:
	using read_callback = std::function<uint8_t(uint32_t)>;
	using write_callback = std::function<void(uint32_t, uint8_t)>;
	using read_word_callback = std::function<uint16_t(uint32_t)>;
	using write_word_callback = std::function<void(uint32_t, uint16_t)>;
	using port_read_callback = std::function<uint8_t()>;
	using port_write_callback = std::function<void(uint8_t)>;
	using analog_callback = std::function<uint16_t()>;

	m37710_cpu_device();
	bool load_internal_rom(const uint8_t* data, std::size_t size);
	void set_memory_callbacks(read_callback read, write_callback write,
	                          read_word_callback read_word = {},
	                          write_word_callback write_word = {});
	void set_port_callbacks(unsigned port, port_read_callback read,
	                        port_write_callback write = {});
	void set_analog_callback(unsigned channel, analog_callback read);
	void reset();
	int execute(int cycles);
	void set_input(int inputnum, bool asserted);
	uint32_t program_counter() const { return m_pg | m_pc; }
	// Exposed for deterministic peripheral regression tests.
	int64_t timer_reload_cycles(unsigned timer) const {
		return timer < 8 ? m_reload[timer] : 0;
	}
...
private:
	...
	void advance_timers(int cycles);

	// internal registers
	template <int Base> uint8_t port_r(uint32_t offset);
	template <int Base> void port_w(uint32_t offset, uint8_t data);
	...
	void da_reg_w(uint32_t offset, uint8_t data);
	void pulse_output_w(uint32_t offset, uint8_t data);
	uint8_t ad_control_r();
	void ad_control_w(uint8_t data);
	void ad_timer_cb();
	uint8_t ad_sweep_r();
	void ad_sweep_w(uint8_t data);
	uint16_t ad_result_r(uint32_t offset);
	...
	uint16_t timer_reg_r(uint32_t offset, uint16_t mem_mask);
	void timer_reg_w(uint32_t offset, uint16_t data, uint16_t mem_mask);
	uint8_t timer_mode_r(uint32_t offset);
	void timer_mode_w(uint32_t offset, uint8_t data);
	uint8_t proc_mode_r(uint32_t offset);
	...
	uint16_t dmac_control_r(uint32_t offset, uint16_t mem_mask);
	void dmac_control_w(uint32_t offset, uint16_t data, uint16_t mem_mask);
	...
	// I/O port callbacks
	std::array<port_read_callback, 11> m_port_in_cb;
	std::array<port_write_callback, 11> m_port_out_cb;

	// A-D callbacks
	std::array<analog_callback, 8> m_analog_cb;
	read_callback m_external_read;
	write_callback m_external_write;
	read_word_callback m_external_read_word;
	write_word_callback m_external_write_word;
	std::array<uint8_t, 0x200> m_internal_ram{};
	std::array<uint8_t, 0x4000> m_internal_rom{};
	...
	void m37710_timer_cb(int which);
