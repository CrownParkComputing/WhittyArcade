...
i960_cpu_device::i960_cpu_device(const machine_config &mconfig, device_type type, const char *tag, device_t *owner, uint32_t clock)
	...
	, m_stalled(false), m_yield_requested(false), m_program_config("program", ENDIANNESS_LITTLE, 32, 32, 0)
...
i80960kb_device::i80960kb_device()
	: i80960kb_device(machine_config{}, "standalone_i960kb", nullptr, 25000000)
{
}
...
void i960_cpu_device::execute_run()
{
	...
	while(m_icount > 0) {
		...
		// A FIFO read can ask the CPU to retry this instruction. The other
		// device runs after this slice, so do not spin on the empty FIFO for
		// the rest of the cycle budget; account the wait as elapsed time and
		// resume at the restored IP on the next scheduler slice.
		if (m_stalled) {
			m_icount = 0;
			break;
		}
...
void i960_cpu_device::standalone_start()
{
	device_start();
	device_reset();
}

void i960_cpu_device::standalone_reset()
{
	device_reset();
}

int i960_cpu_device::standalone_execute(int cycles)
{
	if (cycles <= 0)
		return 0;
	m_icount = cycles;
...
void i960_cpu_device::set_program_callbacks(read_callback read,
...
{
	i960_disassembler disassembler;
	util::disasm_interface::data_buffer bytes(
		[this](offs_t location) {
			return space(AS_PROGRAM).read_byte(location);
		});
	std::ostringstream output;
	const offs_t result = disassembler.disassemble(output, address, bytes, bytes);
	if (length) *length = result & 0xffff;
	return output.str();
}
...
void i960_cpu_device::device_start()
{
	...
	save_item(NAME(m_yield_requested));
