...
{
public:
    using read_callback = address_space::read_callback;
    ...
    void standalone_reset();
    int standalone_execute(int cycles);
    ...
    uint32_t register_value(unsigned index) const {
        return index < 32 ? m_r[index] : 0;
    }
    uint32_t process_controls() const { return m_PC; }
    uint32_t arithmetic_controls() const { return m_AC; }
    std::string disassemble(uint32_t address,
                            std::size_t* length = nullptr) const;
	...
	// End the current scheduling quantum after this instruction without
	// rewinding it. FIFO writers use this when the destination applies
	// back-pressure; unlike i960_stall(), the completed write must not repeat.
	void i960_yield() { m_yield_requested = true; }
