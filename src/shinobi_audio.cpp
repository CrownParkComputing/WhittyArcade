...
struct shinobi_sound_synth::implementation {
    opm_bus         opm_bus_;
    ymfm::ym2151    opm_;
    segapcm         pcm_;
...
shinobi_sound_synth::shinobi_sound_synth()
    : m_impl(std::make_unique<implementation>()) {}

shinobi_sound_synth::~shinobi_sound_synth() = default;

void shinobi_sound_synth::reset() {
    m_impl->opm_.reset();
    m_impl->pcm_.reset();
}

void shinobi_sound_synth::write_control(unsigned port, uint8_t data) {
    if (port == 0) {
...
uint8_t shinobi_sound_synth::read_status() {
    return m_impl->opm_.read_status();
}

void shinobi_sound_synth::advance_timer_clocks(uint32_t clocks) {
    m_impl->opm_bus_.advance_clocks(clocks);
}

void shinobi_sound_synth::generate(int16_t* output, int frames) {
    if (!output || frames <= 0) return;

    constexpr int kOut = shinobi_sound_synth::sample_rate;
    ...
    constexpr double native_rate =
        static_cast<double>(shinobi_sound_synth::chip_clock_hz) / 64.0;
...
void shinobi_sound_synth::set_pcm_rom(const uint8_t* rom, std::size_t bytes) {
    m_impl->pcm_.set_rom(rom, bytes);
}

std::unique_ptr<shinobi_sound_synth> make_shinobi_sound_synth() {
    return std::make_unique<shinobi_sound_synth>();
}
...
shinobi_audio_system::shinobi_audio_system(
    std::unique_ptr<shinobi_sound_synth> synth)
    : m_synth(std::move(synth)) {}

shinobi_audio_system::~shinobi_audio_system() { shutdown(); }

bool shinobi_audio_system::initialize() {
...
void shinobi_audio_system::shutdown() {
    stop();
...
void shinobi_audio_system::start() {
    if (!m_initialized.load() || m_running.load()) return;
    m_running.store(true);
    m_thread = std::thread([this] { audio_loop(); });
}

void shinobi_audio_system::stop() {
    m_running.store(false);
    if (m_thread.joinable()) m_thread.join();
}

void shinobi_audio_system::write_control(unsigned port, uint8_t data) {
    if (!m_synth || port > 0xffu) return;
    std::lock_guard<std::mutex> lock(m_synth_mutex);
    m_synth->write_control(port, data);
}

uint8_t shinobi_audio_system::read_status() {
    if (!m_synth) return 0;
    std::lock_guard<std::mutex> lock(m_synth_mutex);
    return m_synth->read_status();
}

void shinobi_audio_system::advance_timer_clocks(uint32_t clocks) {
    if (!m_synth || clocks == 0) return;
    std::lock_guard<std::mutex> lock(m_synth_mutex);
    m_synth->advance_timer_clocks(clocks);
}

void shinobi_audio_system::set_mix_levels(int master, int music, int effects) {
...
void shinobi_audio_system::fill_buffer(uint32_t buffer) {
    {
        std::lock_guard<std::mutex> lock(m_synth_mutex);
    ...
    const arcade_audio_output::metrics output =
        m_output_normalizer.process_in_place(
            ...
            shinobi_sound_synth::sample_rate,
...
void shinobi_audio_system::audio_loop() {
