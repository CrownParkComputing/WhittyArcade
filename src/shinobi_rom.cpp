...
bool shinobi_roms::complete() const {
...
shinobi_rom_set shinobi_rom_loader::identify_set(const std::string& path) {
    namespace fs = std::filesystem;
    if (path.empty()) return shinobi_rom_set::unknown;
    std::error_code ec;
    if (!fs::exists(path, ec)) return shinobi_rom_set::unknown;
    ...
    return found_main ? shinobi_rom_set::shinobi_us
                      : shinobi_rom_set::unknown;
}

const char* shinobi_rom_loader::set_short_name(shinobi_rom_set set) {
    switch (set) {
    case shinobi_rom_set::shinobi_us: return "shinobi4";
    case shinobi_rom_set::unknown: return "";
    }
    return "";
}

const char* shinobi_rom_loader::set_display_name(shinobi_rom_set set) {
    switch (set) {
    case shinobi_rom_set::shinobi_us: return "Shinobi (Sega System 16B, US)";
    case shinobi_rom_set::unknown:    return "Unknown Shinobi-style set";
    }
    return "Unknown Shinobi-style set";
}

shinobi_rom_load_result shinobi_rom_loader::load(const std::string& path) {
    shinobi_rom_load_result r{};
    r.set = identify_set(path);
    if (r.set == shinobi_rom_set::unknown) {
