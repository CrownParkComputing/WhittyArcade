...
//      bus reads/writes into `shinobi::board`'s per-board memory map.
//   2. `shinobi_machine_t`: the runtime that drives the 68000 each frame
...
struct shinobi_machine_t::sound_cpu {
    z80_t    cpu{};
    uint64_t pins{0};
...
bool shinobi_machine_t::load_roms(const std::string& path) {
    auto result = shinobi_rom_loader::load(path);
    if (!result) {
...
void shinobi_machine_t::run_frame() {
...
uint32_t shinobi_machine_t::main_pc() const {
    return m_main_cpu ? m_main_cpu->program_counter() : 0;
}

void shinobi_machine_t::set_input(const input_state& state) {
    if (!m_ready) return;
